    ...
    return (
        <> 
            <button onClick={onClick} className={`border-yellow-300 last:border-0 border-r-4 h-full w-full`}>
                ...
                {!parkingDetail.isBooked && <div className={`w-10 h-10 rounded-3xl text-sm font-bold flex items-center justify-center m-auto ${parkingDetail.isBooked ? 'bg-red-600' : 'bg-green-600'}`}>{parkingDetail.parkingNumber}</div>}
